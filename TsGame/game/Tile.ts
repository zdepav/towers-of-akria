    ...
    render(ctx: CanvasRenderingContext2D, preRender: boolean): void {
        if (preRender) {
            switch (this.type) {
                case TileType.Empty:
                    ctx.drawImage(
                        Tile.tiles, 0, 0, 64, 64,
                        this.pos.x, this.pos.y, 64, 64
                    )
                    break
                case TileType.Path:
                    ctx.drawImage(
                        Tile.tiles, 0, 64, 64, 64,
                        this.pos.x, this.pos.y, 64, 64
                    )
                    break
                case TileType.Spawn:
                    ctx.drawImage(
                        Tile.tiles, 0, 128, 64, 64,
                        this.pos.x, this.pos.y, 64, 64
                    )
                    break
                case TileType.HQ:
        ...
        return Utils.getImageFromCache('td_tiles').then(
            ...
            () => new Promise<void>(resolve => {
                ...
                new NoiseTextureGenerator(
                    64, 64, '#5BA346', 0.075, 0, 0.25
                ).generateInto(ctx, 0, 0)
                new NoiseTextureGenerator(
                    64, 128, '#B5947E', 0.04, 0, 0.2
                ).generateInto(ctx, 0, 64)

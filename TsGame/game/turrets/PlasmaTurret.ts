...
class PlasmaTurret extends Turret {
    ...
    private static turretDescription =
        'Randomly hits enemies in range, occasionally burning or stunning them'
        ...
        if (this.ready) {
            ...
            if (enemies.length > 0) {
                ...
                this.game.spawnParticle(
                    new PlasmaBeamParticle(this.center.x, this.center.y, e.x, e.y)
                )
                this.cooldown = 0.2
            }
        }
    ...
    static getInfo(type: TurretType): TurretInfo | undefined {
        return new TurretInfo(
            ...
            (type.count * 10).toString(),
            type.water * 5 + '% stun (0.5s), ' + type.fire * 5 + '% burn (1s), '
        )
    }
    ...
    getInfoAfterUpgrade(type: TurretElement): TurretInfo | undefined {
        ...
        switch (type) {
            ...
            case TurretElement.Fire:
                return PlasmaTurret.getInfo(this.type.with(type))
                    ?.withUpgradeNote('improves burning chance')
            case TurretElement.Water:
                return PlasmaTurret.getInfo(this.type.with(type))
                    ?.withUpgradeNote('improves stun chance')
        }
    }
        ...
        return Utils.getImageFromCache('td_tower_AeFW_plasma_strip' + PlasmaTurret.frameCount).then(
            ...
            () => new Promise<void>(resolve => {
                ...
                let tex1a = new CirclesTextureGenerator(
                    64, 64, '#A389FFC0', color1, background, 0.4, 2, 0.7
                )
                let tex1b = new CirclesTextureGenerator(
                    64, 64, '#A389FFC0', color1, background, 0.28, 3, 0.7
                )
                ...
                let tex2a = new CirclesTextureGenerator(
                    64, 64, color2, back2, background, 0.4, 2, 0.1
                )
                let tex2b = new CirclesTextureGenerator(
                    64, 64, color2, back2, background, 0.28, 3, 0.1
                )

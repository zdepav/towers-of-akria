...
class IceTurret extends Turret {
    ...
    private static turretDescription =
        'Freezes all enemies in range, causing them to take damage and slow down'
    ...
    static getInfo(type: TurretType): TurretInfo | undefined {
        return new TurretInfo(
            ...
            (4 + type.count * 3).toString(),
            'freeze(' + (type.air * 0.5 + type.water - 0.5) + ')'
        )
    }
        ...
        return Utils.getImageFromCache('td_tower_AefW_ice').then(
            ...
            () => new Promise<void>(resolve => {
                let tex = new CellularTextureGenerator(
                    64, 64, 64, '#D1EFFF', '#70BECC', CellularTextureType.Cells
                )
                let c = new PreRenderedImage(64, 192)
                let c2 = new PreRenderedImage(64, 64)
                let fill = <CanvasPattern>c2.ctx.createPattern(tex.generateImage(), 'repeat')

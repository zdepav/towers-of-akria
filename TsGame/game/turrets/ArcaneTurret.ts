...
class ArcaneTurret extends Turret {
    ...
    private static turretDescription =
        'Increases damage of all other towers by 25%, can instantly kill any enemy with a' +
        ' cooldown of 16 seconds'

    private frame: number
    private readonly orbits: {
        r1: number
        r2: number
        angle: number
        pos: number
            ...
            if (enemy) {
                for (let i = 0; i < ArcaneTurret.orbitCount; ++i) {
                    ...
                    this.game.spawnParticle(
                        new SparkParticle(enemy.x, enemy.y, ArcaneTurret.orbitColors[i % 4])
                    )
                    this.game.spawnParticle(
                        new SparkParticle(enemy.x, enemy.y, ArcaneTurret.orbitColors[i % 4])
                    )
        ...
        return Utils.getImageFromCache('td_tower_AEFW_arcane').then(
            ...
            () => new Promise<void>(resolve => {
                ...
                let g: ColorSource = new RotatingSource(
                    64, 64, this.prepareGradient(RgbaColor.green), Angle.deg60, 32, 32
                )
                let b: ColorSource = new RotatingSource(
                    64, 64, this.prepareGradient(RgbaColor.blue), Angle.deg120, 32, 32
                )
                let rgb: ColorSource = new BufferedColorSource(
                    64, 64, new AddingSource(64, 64, r, new AddingSource(64, 64, g, b))
                )
                let v: ColorSource = new BufferedColorSource(
                    64, 64, new VelvetTextureGenerator(64, 64, '#FFFFFF00', '#FFFFFF', 0.5)
                )
                let i = 0
                function curve(x: number): number {
                    return Math.cos(
                        i * Angle.deg360 / ArcaneTurret.frameCount + x * Angle.deg360
                    ) / 2 + 0.5
                }
                ...
                for (; i < ArcaneTurret.frameCount; ++i) {
                    ...
                    new CircleSource(
                        64, 64, 32, 32, 24, grad,
                        ArcaneTurret.prepareGround(glass)
                    ).generateInto(c.ctx, i * 64, 0)
                }

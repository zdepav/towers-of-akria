...
class EarthTurret extends Turret {
        ...
        if (this.ready) {
            ...
            if (enemy) {
                this.game.spawnProjectile(
                    new EarthProjectile(this.game, this.center, enemy, this.type.earth)
                )
                this.cooldown = 0.5
            }
        }
    ...
    static getInfo(type: TurretType): TurretInfo | undefined {
        return new TurretInfo(
            ...
            112 + type.earth * 16,
            (15 + type.earth * 5).toString(),
            (1 + type.earth * 2) + '0% to stun for 0.5s'
        )
    }
    ...
    private static preRender1(ctx: CanvasRenderingContext2D, y: number): void {
        ...
        for (const corner of corners) {
            ...
            grad = ctx.createRadialGradient(
                corner.x, y + corner.y, 5, corner.x, y + corner.y, 10
            )
